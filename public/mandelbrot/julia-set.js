...
// Track if actual movement occurred (to avoid render on simple tap)
let juliaHasPanned = false;
const JULIA_DRAG_THRESHOLD = 3;
// Pinch-zoom state for Julia canvas
let juliaPinching = false;
let juliaHasPinched = false;
let juliaPinchStartDist = 0;
let juliaPinchStartScale = 1;
let juliaPinchAnchorX = 0;
let juliaPinchAnchorY = 0;
const juliaActivePointers = new Map();
...
// Helper to get distance between two pointers
function getJuliaPinchDistance(pointers) {
...
// Helper to get midpoint between two pointers (in canvas coordinates)
function getJuliaPinchMidpoint(pointers, rect) {
    const pts = Array.from(pointers.values());
...
function setupJuliaPanZoom() {
    ...
    juliaCanvas.addEventListener("pointerdown", (e) => {
        ...
        // Track this pointer
        juliaActivePointers.set(e.pointerId, { x: e.clientX, y: e.clientY });

        const rect = juliaCanvas.getBoundingClientRect();

        // Check if this is the start of a pinch gesture (2 fingers)
        if (juliaActivePointers.size === 2) {
            juliaPinching = true;
            juliaHasPinched = false;
            juliaPanning = false;

            juliaPinchStartDist = getJuliaPinchDistance(juliaActivePointers);
            juliaPinchStartScale = juliaScreenScale;

            const mid = getJuliaPinchMidpoint(juliaActivePointers, rect);
            juliaPinchAnchorX = mid.x;
            juliaPinchAnchorY = mid.y;

            // Don't call touchJuliaInteraction yet - wait for actual pinch movement
            return;
        }

        // Single finger: start panning
        if (!juliaPinching && juliaActivePointers.size === 1) {
            juliaPanning = true;
            juliaHasPanned = false;
            juliaPanLastX = e.clientX;
            juliaPanLastY = e.clientY;
            juliaCanvas.setPointerCapture(e.pointerId);
            // Don't call touchJuliaInteraction yet - wait for actual pan movement
        }
    });

    juliaCanvas.addEventListener("pointermove", (e) => {
        if (!juliaActivePointers.has(e.pointerId)) return;
        ...
        // Handle pinch zoom
        if (juliaPinching && juliaActivePointers.size === 2) {
            const dist = getJuliaPinchDistance(juliaActivePointers);
            const mid = getJuliaPinchMidpoint(juliaActivePointers, rect);

            // Check if pinch distance changed enough to count as actual pinching
            if (!juliaHasPinched && Math.abs(dist - juliaPinchStartDist) > JULIA_DRAG_THRESHOLD) {
                juliaHasPinched = true;
            }

            if (juliaHasPinched) {
                const zoomFactor = dist / (juliaPinchStartDist || 1);
                const newScale = juliaPinchStartScale * zoomFactor;

                // Calculate new offset to keep pinch anchor stationary
                const prevScale = juliaScreenScale;
                juliaScreenScale = Math.max(1e-6, Math.min(1e6, newScale));

                // Adjust offset so the midpoint stays in place
                juliaScreenOffsetX = mid.x - (juliaPinchAnchorX - juliaScreenOffsetX) * (juliaScreenScale / juliaPinchStartScale);
                juliaScreenOffsetY = mid.y - (juliaPinchAnchorY - juliaScreenOffsetY) * (juliaScreenScale / juliaPinchStartScale);

                // Update anchor for next frame (allows simultaneous pan+zoom)
                juliaPinchAnchorX = mid.x;
                juliaPinchAnchorY = mid.y;
                juliaPinchStartScale = juliaScreenScale;
                juliaPinchStartDist = dist;
        ...
        if (!juliaPanning || juliaPinching) return;

        const dx = e.clientX - juliaPanLastX;
        const dy = e.clientY - juliaPanLastY;

        // Check if we've moved enough to count as actual panning
        if (!juliaHasPanned && (Math.abs(dx) > JULIA_DRAG_THRESHOLD || Math.abs(dy) > JULIA_DRAG_THRESHOLD)) {
            juliaHasPanned = true;
        }
    ...
    function endJuliaPan(e) {
        if (juliaActivePointers.has(e.pointerId)) {
            juliaActivePointers.delete(e.pointerId);
        }

        // End pinch if we drop below 2 fingers
        if (juliaPinching && juliaActivePointers.size < 2) {
            juliaPinching = false;
            if (juliaHasPinched) {
                touchJuliaInteraction();
        ...
        // Only trigger interaction settle if actual panning occurred
        if (juliaHasPanned) {
            touchJuliaInteraction();
        }
        juliaHasPanned = false;
    ...
    juliaCanvas.addEventListener("pointerup", endJuliaPan);
    juliaCanvas.addEventListener("pointercancel", endJuliaPan);
