...
For any value \(c\) that produces a disconnected Julia-set we color that value of \(c\) black in the complex plane. For any value of \(c\) that produces a Julia-set which is completely connected, we color that value blue in the complex plane. If we do this for every pixel on our screen, we get the all-too-famous Mandelbrot set.

In this render I have taken some common artistic liberties with removing the solid infill and initially only coloring the border between the set and its surroundings. The saturation of color is determined from how connected/disconnected the Julia-sets in that region are. You may choose to recolor the infill if you want.
